...
- `.htaccess` (public/.htaccess, copiado a dist/.htaccess): 301 explícitos para rutas legacy. Auditoría pública 2026-08-01 confirmó destino final correcto para `/77/` y `/1old/`; falta repetir con `curl -I` sin `-L` para certificar código `301`. Headers de producción verificados: `Strict-Transport-Security` (HSTS), `Content-Security-Policy` (`upgrade-insecure-requests`), `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` y `X-Frame-Options`. HSTS/CSP ya no son pendientes de producción.
