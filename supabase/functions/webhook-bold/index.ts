...
Deno.serve(
  withTelemetry(FN_NAME, async req => {
    ...
    if (!eraPagado && nuevoEstado === 'pagado') {
      const pagado = await registrarPedidoPagado(supabase, pedidoRow.id, 'wompi', evento.event_id);
      if (pagado.stockOk) {
        await notificarFulfillmentDropship(supabase, pedidoRow.id, pedidoRow.items ?? []);
      } else {
        console.error('webhook-bold: skip dropship por stock_deficit', pedidoRow.id);
      }
