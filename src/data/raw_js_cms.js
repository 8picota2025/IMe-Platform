...
const CMS = {
  ...
  save(products) {
    try { localStorage.setItem(CMS_KEY, JSON.stringify(products)); } catch {}
  },
  add(product) {
  ...
  isLoggedIn() { return sessionStorage.getItem('ime_cms_auth') === '1'; },
  login(pass)  { if (pass === CMS_PASS) { sessionStorage.setItem('ime_cms_auth', '1'); return true; } return false; },
  logout()     { sessionStorage.removeItem('ime_cms_auth'); },
};
...
function renderCatalogFromCMS() {
  ...
  if (window.io) {
    grid.querySelectorAll('.fade-in').forEach(el => { if (window.io) window.io.observe(el); });
  } else {
    setTimeout(() => { grid.querySelectorAll('.fade-in').forEach(el => el.classList.add('visible')); }, 100);
  }
}
...
function initAdmin() {
  ...
  document.getElementById('loginForm')?.addEventListener('submit', e => {
    ...
    if (CMS.login(pass)) {
      ...
      if (errEl) { errEl.textContent = 'Contraseña incorrecta.'; errEl.classList.add('show'); }
    }
  });

  document.getElementById('logoutBtn')?.addEventListener('click', () => {
    CMS.logout(); location.reload();
  });
  ...
  function renderTable() {
    ...
    tbody.querySelectorAll('.act-edit').forEach(btn => btn.addEventListener('click', () => editProduct(btn.dataset.id)));
    tbody.querySelectorAll('.act-del').forEach(btn  => btn.addEventListener('click', () => deleteProduct(btn.dataset.id)));
  }
  ...
  function editProduct(id) {
    ...
    if (preview && wrap) { preview.src = p.img; wrap.style.display = 'block'; }
    // switch tab
  ...
  document.getElementById('productForm')?.addEventListener('submit', e => {
    ...
    if (!data.name || !data.cat || !data.img || !data.desc) {
      alert('Todos los campos son requeridos.'); return;
    }
    if (id) { CMS.update(id, data); } else { CMS.add(data); }
  ...
  document.getElementById('exportBtn')?.addEventListener('click', () => {
    ...
    a.download = 'ime-productos.json'; a.click();
  });
  ...
  document.getElementById('importFile')?.addEventListener('change', function() {
    ...
    reader.onload = ev => {
      try {
        ...
        if (Array.isArray(data)) { CMS.save(data); renderTable(); alert('Importación exitosa: ' + data.length + ' productos.'); }
  ...
  document.getElementById('resetDefaultBtn')?.addEventListener('click', () => {
    if (!confirm('¿Restaurar el catálogo a los productos por defecto? Esta acción no se puede deshacer.')) return;
    CMS.save([...DEFAULT_PRODUCTS]); renderTable(); alert('Catálogo restaurado.');
  });
}
