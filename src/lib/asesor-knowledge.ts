import { IME_EMAIL, IME_WHATSAPP_DISPLAY } from './contacto-oficial.ts';
...
- Canales comerciales publicados: WhatsApp ${IME_WHATSAPP_DISPLAY}, correo ${IME_EMAIL} y formulario de contacto del sitio.
...
- Published commercial channels: WhatsApp ${IME_WHATSAPP_DISPLAY}, email ${IME_EMAIL} and the website contact form.
...
export function esConsultaContacto(texto: string): boolean {
  return CONTACT_QUERY_REGEX.test(texto);
}
...
export function buildAsesorStaticFallback(
  ...
  if (locale === 'en') {
    ...
    if (wantsServices || wantsContact) {
      return `I-ME states that it provides sale and distribution of biomedical equipment, installation and commissioning, preventive and corrective technical support, calibration and metrological verification, spare parts and consumables, indicative financing, and biomedical advisory. The published commercial channels are WhatsApp ${IME_WHATSAPP_DISPLAY}, email ${IME_EMAIL}, and the website contact form.`;
    }
