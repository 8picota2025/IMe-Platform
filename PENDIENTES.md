...
- [ ] `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` / `PUBLIC_TURNSTILE_SITE_KEY` (mismo valor que `TURNSTILE_SITE_KEY`): secretos GitHub existen y la site key pública está en prod (`0x4AAAAAADn55axRM58KEtfM`). TODO_CLIENTE: en Cloudflare Turnstile Dashboard confirmar hostnames `i-me.com.co` y `www.i-me.com.co` (un hostname faltante suele pintar recuadro gris / error 110200 en Android Chrome), widget managed (checkbox), y que el secret de siteverify es el par de esa site key. Un par o allowlist incorrectos muestran `asesor.verificacion` (403), no 503. Bypass de emergencia `ASESOR_TURNSTILE_BYPASS=true` existe en Edge y debe permanecer OFF.
