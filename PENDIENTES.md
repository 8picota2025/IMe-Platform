...
- [ ] `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` / `PUBLIC_TURNSTILE_SITE_KEY` (mismo valor que `TURNSTILE_SITE_KEY`): siguen usándose en evento/leads/propuesta. El chat IMEIA **no** exige Turnstile (2026-09-17): default off en cliente y Edge. Para reactivar el muro: `PUBLIC_ASESOR_TURNSTILE=true` (build) **y** `ASESOR_TURNSTILE_REQUIRED=true` (secreto Edge). `ASESOR_TURNSTILE_BYPASS=true` salta siteverify aunque REQUIRED esté on. Rate-limit IP/sesión del asesor se mantiene. 2026-09-17 noche: poll 180s + resume del mismo `turnId` para que Reintentar muestre `reply_texto` ya escrito (evita POLL_TIMEOUT/INVOKE_ABORT con respuesta en BD).
